...
def interpolate_daily(dates: np.ndarray, values: np.ndarray) -> tuple[np.ndarray, np.ndarray]:
    ...
    start_date: np.datetime64 = dates[0]  # type: ignore[assignment]
    end_date: np.datetime64 = dates[-1]  # type: ignore[assignment]
    ...
    for i in range(first_valid, last_valid + 1):
        if np.isnan(new_values[i]):
            ...
            if len(prev_idx) > 0 and len(next_idx) > 0:
                ...
                new_values[i] = new_values[prev_idx] + weight * (
                    new_values[next_idx] - new_values[prev_idx]
                )
...
def calculate_macro_calories(
    ...
    total_potential_cal = (
        potential_protein_cal
        + potential_carbs_cal
        + potential_saturated_fat_cal
        + potential_other_fat_cal
    )

    # Calculate adjustment factor (avoid division by zero)
    adjustment_factor = np.where(total_potential_cal > 0, total_calories / total_potential_cal, 1.0)

    # Handle inf/-inf values
    adjustment_factor = np.where(np.isfinite(adjustment_factor), adjustment_factor, 1.0)
...
def create_date_range(
    start_date: np.datetime64, end_date: np.datetime64, freq: str = "D"
) -> np.ndarray:
    """
    ...
    delta = (end_date - start_date).astype("timedelta64[D]")
    num_days = int(delta / np.timedelta64(1, "D")) + 1
...
def fill_missing_values(values: np.ndarray, method: str = "linear") -> np.ndarray:
    ...
    if method == "linear":
        ...
        raise ValueError(
            f"Unknown fill method: {method}. Use 'linear', 'forward', 'backward', or 'mean'"
        )
