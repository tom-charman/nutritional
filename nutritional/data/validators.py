...
class QualityReport(TypedDict):
    ...
    date_gaps: list[tuple[Any, Any, int]]
...
def validate_date_range(
    dates: np.ndarray,
    min_date: np.datetime64 | None = None,
    max_date: np.datetime64 | None = None,
) -> tuple[bool, str]:
    """
...
def check_data_quality(data: dict, verbose: bool = True) -> QualityReport:
    """
    ...
    if verbose:
        ...
        if report["missing_percentages"]:
            for col, pct in sorted(
                report["missing_percentages"].items(), key=lambda x: x[1], reverse=True
            )[:10]:
        ...
        if report["date_gaps"]:
            ...
            for gap_start, gap_end, gap_days in report["date_gaps"][:5]:
                ...
                print(
                    f"{status} Gap of {gap_days} days: {gap_start} to {gap_end}"
                )  # pragma: no cover
...
def check_required_columns_for_plot(data: dict, plot_name: str) -> tuple[bool, list[str]]:
    ...
    plot_requirements = {
        ...
        "nutrients": ["Saturated Fat g", "Sugar g", "Fibre g", "Salt g", "Calcium mg"],
    }

    if plot_name not in plot_requirements:
        raise ValueError(
            f"Unknown plot name: {plot_name}. "
...
def suggest_data_fixes(data: dict) -> list[str]:
    ...
    high_missing = [
        (col, pct) for col, pct in quality_report["missing_percentages"].items() if pct > 20
    ]
    if high_missing:
        suggestions.append(
    ...
    total_days = (
        quality_report["date_range"][1] - quality_report["date_range"][0]
    ) / np.timedelta64(1, "D")
    if total_days < 30:
        suggestions.append(
