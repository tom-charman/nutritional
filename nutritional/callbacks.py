...
from nutritional.data.loaders import get_data_source, filter_by_date_range
...
def update_dashboard(stored_data, start_date, end_date, rolling_window):
    ...
    avg_weight = f"{stats['avg_weight_morning']:.1f}" if stats['avg_weight_morning'] is not None else "N/A"
    ...
    source_info = f"Data source: {source} | Date range: {date_range_str} | Records: {data_points} | Last updated: {last_updated}"
    
    return (fig1, fig2, fig3, 
            avg_cals, avg_weight, avg_protein, data_points,
            source_info, None)
