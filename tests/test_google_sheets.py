...
def test_client_init_with_explicit_path(tmp_path, monkeypatch):
    ...
    with patch(
        "nutritional.data.google_sheets.service_account.Credentials.from_service_account_file",
...
def test_get_last_modified_returns_timestamp(tmp_path):
    ...
    mock_drive_service.files().get().execute.return_value = {"modifiedTime": expected_time}
